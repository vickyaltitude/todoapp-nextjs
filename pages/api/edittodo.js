...
async function handler(req,res){
 ...
 if(req.method === 'PATCH'){
    ...
    const result = await meetupCollections.findOneAndUpdate({_id: new ObjectId(dataTodos.id)},{$set:{todo : dataTodos.todoupdated}});
    console.log(result);
    connectionDB.close();
    res.status(200).json({message: 'Meetup edited Fetched!'})
